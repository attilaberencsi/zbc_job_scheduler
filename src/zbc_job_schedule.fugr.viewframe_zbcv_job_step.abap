*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZBCV_JOB_STEP
*   generation date: 20.10.2016 at 10:19:36 by user BERENCSI_A
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZBCV_JOB_STEP       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZBCV_JOB_STEP'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZBCV_JOB_STEP_EXTRACT
TOTAL = ZBCV_JOB_STEP_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZBCV_JOB_STEP'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZBCV_JOB_STEP-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZBCV_JOB_STEP_EXTRACT
TOTAL = ZBCV_JOB_STEP_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZBCV_JOB_STEP-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZBCV_JOB_STEP'
...
TOTAL = ZBCV_JOB_STEP_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZBCV_JOB_STEP-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZBCV_JOB_STEP-UPD_FLAG NE SPACE.
